...
# Bash is selected by the script shebang.
...
# -------------------------- Directories ---------------------------
# Create lock base (shared FS)
if ! mkdir -p "${LOCKDIR_BASE}"; then
  echo "ERROR: Cannot create LOCKDIR_BASE: ${LOCKDIR_BASE}" >&2
  exit 1
fi

# Work under the sample dir (explicit guard)
if ! cd "${SAMPLE_DIR}"; then
  echo "ERROR: SAMPLE_DIR not found or not accessible: ${SAMPLE_DIR}" >&2
  exit 1
fi

# Create output subdirs one-by-one (avoid fragile continuations)
mkdir -p "array_benchmarking/k2_hits"
mkdir -p "array_benchmarking/for_competitive"
mkdir -p "array_benchmarking/k2_classification"
mkdir -p "array_benchmarking/logs"
...
if [[ -z "${PBS_ARRAY_INDEX:-}" ]]; then
  echo "ERROR: SLURM_ARRAY_TASK_ID is unset (are you running as an array job?)" >&2
  exit 1
fi
SAMPLE="$(sed -n "${SLURM_ARRAY_TASK_ID}p" "${SAMPLES_LIST}")"
if [[ -z "${SAMPLE}" ]]; then
  echo "ERROR: No sample on line ${SLURM_ARRAY_TASK_ID} of ${SAMPLES_LIST}" >&2
  exit 1
fi
...
purge_stale_locks() {
  ...
  for lf in "${ARRAY_LOCKDIR}"/*.lock; do
    ...
    local state=$( qstat -f "$base" 2>/dev/null | awk -F= '/\bjob_state\b/ {gsub(/[ \t]/,"",$2); print $2; exit}' )
...
while :; do
  ...
  # Optional wait limit (disabled by default)
  # now=$(date +%s)
...
echo "[${SLURM_ARRAY_TASK_ID}] Starting Kraken2 at: $(date)"
set +e
