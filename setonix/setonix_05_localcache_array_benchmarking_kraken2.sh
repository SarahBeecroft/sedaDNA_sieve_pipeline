...
# ----------------------- Slurm directives ---------------------------
#SBATCH --job-name=k2_array_localdb_optA
#SBATCH --mail-user=you@utas.edu.au
...
# Slurm placement is left to the scheduler; cache reuse is handled by the script.
# Merge stdout/stderr
...
LOCKFILE="${ARRAY_LOCKDIR}/${SLURM_JOB_ID}.lock"
...
WORK="${TMPDIR:-/tmp}/k2_${ARRAY_ID}_${SLURM_ARRAY_TASK_ID}"
mkdir -p "${WORK}"
...
CLASSIFIED_OUT="${OUT_CLASS}/classified-${SAMPLE_BN}-${SLURM_JOB_ID}.fq"
UNCLASS_OUT="${OUT_UNCL}/unclassified-${SAMPLE_BN}-${SLURM_JOB_ID}.fq"
REPORT_OUT="${OUT_CLF}/report-${SAMPLE_BN}-${SLURM_JOB_ID}.txt"
OUTPUT_OUT="${OUT_CLF}/classifications-${SAMPLE_BN}-${SLURM_JOB_ID}.txt"
LOG_OUT="${OUT_LOGS}/${SAMPLE_BN}-${SLURM_JOB_ID}.log"
